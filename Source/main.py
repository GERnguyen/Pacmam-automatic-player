...
pygame.font.init()
font_path = os.path.join(os.path.dirname(__file__), 'Fonts', 'Grand9K_Pixel.ttf')
my_font = pygame.font.Font(font_path, 30)
my_font_2 = pygame.font.Font(font_path, 100)
...
def Draw(_screen, level) -> None:
    ...
    for food in _food:
        food.draw(_screen)
    for ghost in _ghost:
        ghost.draw(_screen, level)

    PacMan.draw(_screen, level, True)
...
def generate_Ghost_new_position(_ghost, _type: int = 0) -> list[list[int]]:
    ...
    if _type == 1:
        ...
        for idx in range(len(_ghost)):
            ...
            (nr, nc), path = Ghost_move_level4(_map, start_row, start_col, end_row, end_col, N, M)
            _ghost_new_position.append((nr, nc))
        ...
        if delay <= 0:
            if is_moving:
                ...
                if Level == 1 or Level == 2:
                    if len(result) <= 0:
                        result = search.execute(ALGORITHMS=ALGORITHMS[a][b])
                        if len(result) > 0:
                            result.pop(0)
                    ...
                    result = search.execute(ALGORITHMS=ALGORITHMS[a][b], visited=_visited)
                    new_PacMan_Pos = result[0] if len(result) > 0 else []
                    _visited[row][col] += 1

                elif Level == 4 and len(_food_Position) > 0:
                    try:
                        new_PacMan_Pos, result = search.execute(ALGORITHMS=ALGORITHMS[a][b], depth=4, Score=Score)
