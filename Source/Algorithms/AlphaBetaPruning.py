...
def AlphaBetaAgent(_map, pac_row, pac_col, N, M, depth, Score):
    ...
    def min_value(_map, _pac_row, _pac_col, _N, _M, _depth, score, path, alpha, beta):
                ...
                if _map[row][col] == MONSTER:
                    for [_d_r, _d_c] in DDX:
                        ...
                        if isValid2(_map, _new_r, _new_c, _N, _M):
                            ...
                            _map[row][col] = EMPTY
                            val, p = max_value(_map, _pac_row, _pac_col, _N, _M, _depth - 1, score, path, alpha, beta)
                            if val < v:
                                v = val
                                best_path = p
    ...
    def max_value(_map, _pac_row, _pac_col, _N, _M, _depth, score, path, alpha, beta):
        if terminal(_map, _pac_row, _pac_col, _N, _M, _depth):
            return evaluationFunction(_map, _pac_row, _pac_col, _N, _M, score), path
        ...
        for [_d_r, _d_c] in DDX:
            ...
            if isValid(_map, _new_r, _new_c, _N, _M):
                ...
                if state == FOOD:
                    ...
                    score -= 1
                val, p = min_value(_map, _new_r, _new_c, _N, _M, _depth - 1, score, path + [[_new_r, _new_c]], alpha, beta)
                if val > v:
                    v = val
                    best_path = p
                _map[_new_r][_new_c] = state
    ...
    for [d_r, d_c] in DDX:
        ...
        if isValid(_map, new_r, new_c, N, M):
            ...
            if _state == FOOD:
                ...
                Score -= 1
            val, path = min_value(_map, new_r, new_c, N, M, depth, Score, [[pac_row, pac_col], [new_r, new_c]], alpha, beta)
